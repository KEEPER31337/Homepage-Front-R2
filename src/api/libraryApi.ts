...
const useGetBookListQuery = ({ searchType, search, page, size }: BookListSearch) => {
  const fetcher = () =>
    axios.get('/books', { params: { searchType, search, page, size } }).then(({ data }) => {
  ...
  return useQuery<{ content: BookInfo[]; totalElement: number }>(
    libraryKeys.bookList({ searchType, search, page, size }),
    fetcher,
  );
};
