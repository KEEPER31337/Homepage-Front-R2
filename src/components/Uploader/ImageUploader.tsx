...
const ImageUploader = ({
  ...
  return (
    <div className="flex h-full flex-col space-y-[10px]">
      ...
      <div
        {...getRootProps()}
        className={` ${isDragActive ? 'bg-pointBlue/30' : ''} ${thumbnailBase64 ? '' : 'border-2'} relative flex h-full items-center justify-center border-dashed !border-pointBlue/30 hover:opacity-70`}
      >
        <input {...getInputProps()} />
        {thumbnailBase64 ? (
          <img
